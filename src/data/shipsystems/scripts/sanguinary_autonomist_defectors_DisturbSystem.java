...
import src.data.scripts.plugins.sanguinary_autonomist_defectors_DisruptSystemEffect;

public class sanguinary_autonomist_defectors_DisturbSystem extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, ShipSystemStatsScript.State state, float effectLevel) {
        ...
        if (effectLevel == 1f) {
            ...
            for (ShipAPI target : list) {
                ...
                sanguinary_autonomist_defectors_DisruptSystemEffect.putTELEMETRY(target);
            }
        }
        if (effectLevel > 0) {
            if (sprite == null) {
                sprite = Global.getSettings().getSprite("ping", "sanguinary_autonomist_defectors_tagPing");
