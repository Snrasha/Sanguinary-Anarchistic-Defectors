...
import src.data.utils.Forg_Tags;
import src.data.utils.Forg_themes;

public class Forg_ThemeGenerator extends BaseThemeGenerator {

    public static enum ForgottenSystemType {
       
        
        DESTROYED(Forg_Tags.THEME_FORGOTTEN_DESTROYED, "$forgottenDestroyed"),
        SUPPRESSED(Forg_Tags.THEME_FORGOTTEN_SUPPRESSED, "$forgottenSuppressed"),
        RESURGENT(Forg_Tags.THEME_FORGOTTEN_RESURGENT, "$forgottenResurgent"),;
    ...
    public void generateForSector(ThemeGenContext context, float allowedSectorFraction) {
        ...
        for (int i = 0; i < num && i < constellations.size(); i++) {
            ...
            context.majorThemes.put(c, Forg_themes.FORGOTTEN);
            numUsed++;
            ...
            for (int j = 0; j < numMain; j++) {
                ...
                data.system.addTag(Forg_Tags.THEME_FORGOTTEN);
                data.system.addTag(Forg_Tags.THEME_FORGOTTEN_MAIN);
                ...
                switch (type) {
                    case DESTROYED:
                        {
                            Forg_SeededFleetManager fleets = new Forg_SeededFleetManager(data.system, 3, 8, 1, 2, 0.05f);
                            data.system.addScript(fleets);
                            break;
                        }
                    case SUPPRESSED:
                        {
                            Forg_SeededFleetManager fleets = new Forg_SeededFleetManager(data.system, 7, 12, 4, 12, 0.25f);
                            data.system.addScript(fleets);
                            ...
                            if (j == 0 && !addSuppressedStation.isEmpty()) {
                                ...
                                for (CampaignFleetAPI station : stations) {
                                    ...
                                    Forg_StationFleetManager activeFleets = new Forg_StationFleetManager(
            ...
            for (StarSystemData data : systems) {
                ...
                data.system.addTag(Forg_Tags.THEME_FORGOTTEN);
                data.system.addTag(Forg_Tags.THEME_FORGOTTEN_SECONDARY);
                data.system.addTag(type.getTag());
                forgottenSystems.add(data);

                if (random.nextFloat() < 0.5f) {
                    Forg_SeededFleetManager fleets = new Forg_SeededFleetManager(data.system, 1, 3, 1, 2, 0.05f);
                    data.system.addScript(fleets);
                }
            }
        }
    ...
    public void addDefenders(List<StarSystemData> systemData) {
        for (StarSystemData data : systemData) {
            float mult = 1f;
            if (data.system.hasTag(Forg_Tags.THEME_FORGOTTEN_SECONDARY)) {
                mult = 0.5f;
            }
    ...
    public List<CampaignFleetAPI> addBattlestations(StarSystemData data, float chanceToAddAny, int min, int max,
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (loc != null) {
                ...
                Forg_SeededFleetManager.addForgottenSurveyDataDrops(random, fleet, mult);
