...
public class Forg_AssignmentAI implements EveryFrameScript {
	...
	public Forg_AssignmentAI(CampaignFleetAPI fleet, StarSystemAPI homeSystem, SectorEntityToken source) {
		this.fleet = fleet;
	...
	protected void giveInitialAssignments() {
		...
		if ((playerInSameLocation || (float) Math.random() < 0.1f) && source != null) {
			...
			SectorEntityToken target = Forg_SeededFleetManager.pickEntityToGuard(new Random(), homeSystem, fleet);
			if (target != null) {
