...
public class sanguinary_autonomist_defectors_StationFleetManager extends SourceBasedFleetManager {
    ...
    public sanguinary_autonomist_defectors_StationFleetManager(SectorEntityToken source, float thresholdLY, int minFleets, int maxFleets, float respawnDelay,
    ...
    protected CampaignFleetAPI spawnFleet() {
        ...
        sanguinary_autonomist_defectors_SeededFleetManager.initSADFleetProperties(random, fleet);

        fleet.setLocation(source.getLocation().x, source.getLocation().y);
        fleet.setFacing(random.nextFloat() * 360f);

        fleet.addScript(new sanguinary_autonomist_defectors_AssignmentAI(fleet, (StarSystemAPI) source.getContainingLocation(), source));
