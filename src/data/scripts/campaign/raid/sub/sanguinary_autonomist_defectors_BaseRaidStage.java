...
import src.data.scripts.campaign.raid.sanguinary_autonomist_defectors_raidIntel.sanguinary_autonomist_defectors_RaidStage;

public class sanguinary_autonomist_defectors_BaseRaidStage implements sanguinary_autonomist_defectors_RaidStage {
	...
	public sanguinary_autonomist_defectors_BaseRaidStage(sanguinary_autonomist_defectors_raidIntel raid) {
		this.intel = raid;
	}
	...
	public List<RouteData> getStragglers(List<RouteData> routes, SectorEntityToken dest, float maxRange) {
		...
		for (RouteData route : routes) {
			...
			if (fleet != null) {
				...
				if (route.getCurrent() != null && sanguinary_autonomist_defectors_AssembleStage.WAIT_STAGE.equals(route.getCurrent().custom)) {
					waiting = true;
				}
				if (!waiting) {
					stragglers.add(route);
