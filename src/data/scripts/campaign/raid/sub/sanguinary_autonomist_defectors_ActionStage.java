...
import src.data.scripts.campaign.raid.sanguinary_autonomist_defectors_raidManager;
import src.data.scripts.campaign.raid.sanguinary_autonomist_defectors_raidIntel;


public class sanguinary_autonomist_defectors_ActionStage extends sanguinary_autonomist_defectors_BaseRaidStage implements BaseAssignmentAI.FleetActionDelegate {
	public sanguinary_autonomist_defectors_ActionStage(sanguinary_autonomist_defectors_raidIntel raid, MarketAPI target) {
		super(raid);
		this.target = target;
...
//		}
		...
		if (status != sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.ONGOING) return;
		
		boolean inSpawnRange = sanguinary_autonomist_defectors_RouteManager.isPlayerInSpawnRange(target.getPrimaryEntity());
		...
		if (!target.isInEconomy() || !target.isPlayerOwned()) {
			status = sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.FAILURE;
			removeMilScripts();
	...
	public String getRaidActionText(CampaignFleetAPI fleet, MarketAPI market) {
		sanguinary_autonomist_defectors_raidIntel intel = ((sanguinary_autonomist_defectors_raidIntel)this.intel);
		sanguinary_autonomist_defectors_raidManager.PunExGoal goal = intel.getGoal();
		if (goal == sanguinary_autonomist_defectors_raidManager.PunExGoal.BOMBARD) {
	...
	public void performRaid(CampaignFleetAPI fleet, MarketAPI market) {
		...
		status = sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.SUCCESS;
		...
		if (goal == sanguinary_autonomist_defectors_raidManager.PunExGoal.BOMBARD) {
			...
			if (cost <= maxCost) {
				...
				intel.setOutcome(sanguinary_autonomist_defectors_raidIntel.PunExOutcome.SUCCESS);
			} else {
				intel.setOutcome(sanguinary_autonomist_defectors_raidIntel.PunExOutcome.BOMBARD_FAIL);
				status = sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.FAILURE;
			...
			if (raidSuccess) {
				...
				intel.setOutcome(sanguinary_autonomist_defectors_raidIntel.PunExOutcome.RAID_FAIL);
				status = sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.FAILURE;
...
//		}
		...
		if (intel.getOutcome() != null) {
			if (status == sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.SUCCESS) {
	...
	protected void autoresolve() {
		...
		if (defensiveStr >= str) {
			...
			sanguinary_autonomist_defectors_raidIntel intel = ((sanguinary_autonomist_defectors_raidIntel)this.intel);
			intel.setOutcome(sanguinary_autonomist_defectors_raidIntel.PunExOutcome.TASK_FORCE_DEFEATED);
			return;
		}
	...
	protected void updateRoutes() {
		...
		((sanguinary_autonomist_defectors_raidIntel)intel).sendEnteredSystemUpdate();
		
		List<sanguinary_autonomist_defectors_RouteManager.RouteData> routes = sanguinary_autonomist_defectors_RouteManager.getInstance().getRoutesForSource(intel.getRouteSourceId());
		for (sanguinary_autonomist_defectors_RouteManager.RouteData route : routes) {
			if (target.getStarSystem() != null) { // so that fleet may spawn NOT at the target
				route.addSegment(new sanguinary_autonomist_defectors_RouteManager.RouteSegment(Math.min(5f, untilAutoresolve), target.getStarSystem().getCenter()));
			}
			route.addSegment(new sanguinary_autonomist_defectors_RouteManager.RouteSegment(1000f, target.getPrimaryEntity()));
		}
	}
	...
	public void showStageInfo(TooltipMakerAPI info) {
		...
		if (status == sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.ONGOING && curr == index) {
		...
		if (intel.getOutcome() != null) {
			...
			case RAID_FAIL:
				...
				break;
			case SUCCESS:
				if (intel.goal == sanguinary_autonomist_defectors_raidManager.PunExGoal.BOMBARD) {
		...
		} else if (status == sanguinary_autonomist_defectors_raidIntel.RaidStageStatus.SUCCESS) {			
