...
public class Forg_Gen implements SectorGeneratorPlugin {

    @Override
    public void generate(SectorAPI sector) {
        initFactionRelationships(sector);
        ThemeGenContext context = new ThemeGenContext();
        Forg_ThemeGenerator gen = new Forg_ThemeGenerator();
    ...
    private static void initFactionRelationships(SectorAPI sector) {
        ...
        FactionAPI forgotten = sector.getFaction("forgotten");

        List<FactionAPI> factionList = sector.getAllFactions();
        factionList.remove(forgotten);

        for (FactionAPI faction : factionList) {
            forgotten.setRelationship(faction.getId(), RepLevel.HOSTILE);
        }
